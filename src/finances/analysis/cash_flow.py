...
from ..core.currency import milliunits_to_cents, cents_to_dollars_str
...
class CashFlowAnalyzer:
    ...
    def load_data(self, ynab_cache_dir: Optional[Path] = None) -> None:
        ...
        if not accounts_file.exists() or not transactions_file.exists():
            raise FileNotFoundError(
                f"YNAB data not found in {ynab_cache_dir}. "
                "Run 'finances ynab sync-cache' first."
            )
        ...
        cash_transactions = [
            ...
            if t.get('account_name') in self.config.cash_accounts
            and t['date'] >= self.config.start_date
        ]
        ...
        complete_balances = {}
        last_balances = {acc: 0 for acc in self.config.cash_accounts}
    ...
    def _calculate_moving_averages(self) -> None:
        ...
        self.df['MA_7'] = self.df['Total'].rolling(
            window=self.config.short_ma_window, min_periods=1
        ).mean()
        self.df['MA_30'] = self.df['Total'].rolling(
            window=self.config.medium_ma_window, min_periods=1
        ).mean()
        self.df['MA_90'] = self.df['Total'].rolling(
            window=self.config.long_ma_window, min_periods=1
        ).mean()
    ...
    def _calculate_monthly_aggregates(self) -> None:
        ...
        self.monthly_df.columns = [
            'Mean_Balance', 'Min_Balance', 'Max_Balance', 'End_Balance', 'Net_Change'
        ]
    ...
    def generate_dashboard(self, output_dir: Optional[Path] = None) -> Path:
        ...
        plt.suptitle(
            'Comprehensive Cash Flow Analysis Dashboard',
    ...
    def _create_main_trend_panel(self, ax) -> None:
        ...
        ax.plot(self.df.index, self.df['Total'], alpha=0.3, color='gray',
                linewidth=0.5, label='Daily Balance')
        ax.plot(self.df.index, self.df['MA_7'], color='#2E86AB',
                linewidth=1.5, label='7-Day MA')
        ax.plot(self.df.index, self.df['MA_30'], color='#A23B72',
                linewidth=2, label='30-Day MA')
        ax.plot(self.df.index, self.df['MA_90'], color='#F18F01',
                linewidth=2.5, label='90-Day MA')
        ax.plot(self.df.index, self.trend_stats['trend_line'], 'r--',
                alpha=0.7, linewidth=1, label='Trend Line')
    ...
    def _create_monthly_flow_panel(self, ax) -> None:
        ...
        ax.bar(self.monthly_df.index, self.monthly_df['Net_Change'],
               color=colors, alpha=0.7)
        ax.axhline(y=0, color='black', linestyle='-', linewidth=0.5)
        ax.axhline(y=monthly_burn_rate, color='blue', linestyle='--', alpha=0.7,
    ...
    def _create_volatility_panel(self, ax) -> None:
        ...
        ax.fill_between(self.monthly_df.index,
                        self.monthly_df['Min_Balance'], self.monthly_df['Max_Balance'],
                        alpha=0.3, color='#2E86AB', label='Monthly Range')
        ax.plot(self.monthly_df.index, self.monthly_df['Mean_Balance'],
                color='#2E86AB', linewidth=2, label='Monthly Average')
        ax.plot(self.monthly_df.index, self.monthly_df['End_Balance'],
                color='#A23B72', linewidth=1, linestyle='--', label='Month-End Balance')
    ...
    def _create_velocity_panel(self, ax) -> None:
        ...
        ax.fill_between(self.df.index, 0, rolling_change,
    ...
    def _create_composition_panel(self, ax) -> None:
        ...
        if positive_accounts:
            ax.stackplot(self.df.index,
                         *[self.df[acc] if acc in self.df.columns else 0
                           for acc in positive_accounts],
