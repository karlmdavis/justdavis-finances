...
from .grouper import group_orders, GroupingLevel, get_order_candidates
from .scorer import MatchScorer, MatchType, ConfidenceThresholds
...
class SimplifiedMatcher:
    ...
    def match_transaction(self, ynab_tx: Dict[str, Any], account_data: Dict[str, Tuple[pd.DataFrame, pd.DataFrame]]) -> Dict[str, Any]:
        """
        ...
        for account_name, (retail_df, digital_df) in account_data.items():
            if retail_df.empty:
                continue
        ...
        if best_match and best_match['match_method'] == 'split_payment':
            ...
            self.split_matcher.record_match(
                ynab_tx['id'],
                order['order_id'],
                order.get('matched_item_indices', [])
            )
    ...
    def _find_complete_matches(self, ynab_amount: int, ynab_date: date, orders_df: pd.DataFrame, account_name: str) -> List[Dict[str, Any]]:
        ...
        for candidate_type, candidate_list in candidates.items():
    ...
    def _find_split_payment_matches(self, ynab_tx: Dict, ynab_amount: int, ynab_date: date, orders_df: pd.DataFrame, account_name: str) -> List[Dict[str, Any]]:
        ...
        for order_id, order_data in complete_orders.items():
            ...
            if split_match and ConfidenceThresholds.meets_threshold(split_match['confidence'], MatchType.SPLIT_PAYMENT):
