...
class AppleMatcher:
    ...
    def match_single_transaction(self,
        ...
        print(f"Matching transaction {tx_id}: {format_cents(tx_amount_cents)} on {tx_date.strftime('%Y-%m-%d')}")
    ...
    def _find_exact_match(self,
        ...
        for _, receipt in same_date_receipts.iterrows():
            if receipt['total'] == tx_amount:
                print(f"  Found exact match: Receipt {receipt['order_id']} for {format_cents(receipt['total'])}")
    ...
    def _find_date_window_match(self,
        ...
        if best_match:
            print(f"  Found date window match: Receipt {best_match['order_id']} for {format_cents(best_match['total'])}, {best_date_diff} days off")
