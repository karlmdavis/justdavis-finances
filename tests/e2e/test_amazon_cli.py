...
@pytest.mark.e2e
@pytest.mark.amazon
def test_amazon_match_with_date_range():
    """Test date filtering in `finances amazon match`."""
    with tempfile.TemporaryDirectory() as tmpdir:
        tmpdir = Path(tmpdir)
        ...
        # Run match with specific date range
        result = subprocess.run(
            [
                "uv",
                "run",
        ...
        # Verify output file contains correct date range
        output_dir = tmpdir / "amazon" / "transaction_matches"
        output_files = list(output_dir.glob("*_amazon_matching_results.json"))
        ...
        assert data["metadata"]["start_date"] == "2025-09-15"
        assert data["metadata"]["end_date"] == "2025-09-20"
...
@pytest.mark.e2e
@pytest.mark.amazon
def test_amazon_match_output_directory():
    """Test custom output directory for `finances amazon match`."""
    with tempfile.TemporaryDirectory() as tmpdir:
        tmpdir = Path(tmpdir)
        ...
        # Run match with custom output directory
        result = subprocess.run(
            [
                "uv",
                "run",
        ...
        # Verify output was written to custom directory
        output_files = list(custom_output.glob("*_amazon_matching_results.json"))
        assert len(output_files) > 0, "No output files in custom directory"
...
@pytest.mark.e2e
@pytest.mark.amazon
def test_amazon_unzip_with_account_filter():
    """Test account filtering in `finances amazon unzip`."""
    with tempfile.TemporaryDirectory() as tmpdir:
        tmpdir = Path(tmpdir)

        # Create download directory with multiple account ZIPs
        download_dir = tmpdir / "downloads"
        download_dir.mkdir()

        # Create test data
        test_orders = generate_synthetic_amazon_orders(num_orders=3)

        for account in ["karl", "erica"]:
            # Create CSV
            csv_file = tmpdir / f"temp_{account}.csv"
            with open(csv_file, "w", newline="") as f:
                if test_orders:
                    writer = csv.DictWriter(f, fieldnames=test_orders[0].keys())
                    writer.writeheader()
            ...
            # Create ZIP
            zip_path = download_dir / f"amazon_orders_{account}.zip"
            with zipfile.ZipFile(zip_path, "w") as zip_ref:
                zip_ref.write(csv_file, "Retail.OrderHistory.1.csv")
        ...
        # Run unzip with account filter (only karl)
        result = subprocess.run(
            [
                "uv",
                "run",
        ...
        # Verify only karl's data was extracted
        karl_dirs = list(output_dir.glob("*_karl_amazon_data"))
        erica_dirs = list(output_dir.glob("*_erica_amazon_data"))

        assert len(karl_dirs) > 0, "Karl's data should be extracted"
        assert len(erica_dirs) == 0, "Erica's data should be filtered out"


@pytest.mark.e2e
@pytest.mark.amazon
def test_amazon_match_with_verbose_output():
    """Test verbose output flag in `finances amazon match`."""
    with tempfile.TemporaryDirectory() as tmpdir:
        tmpdir = Path(tmpdir)
        ...
        assert "Amazon Transaction Matching" in result.stdout
        assert "Date range:" in result.stdout
        assert "2025-09-01" in result.stdout
        assert "2025-09-30" in result.stdout


@pytest.mark.e2e
@pytest.mark.amazon
def test_amazon_match_with_split_disabled():
    """Test disabling split payment matching in `finances amazon match`."""
    with tempfile.TemporaryDirectory() as tmpdir:
        tmpdir = Path(tmpdir)
        ...
        # Run match with split payments disabled
        result = subprocess.run(
            [
                "uv",
                "run",
        ...
        # Verify output file contains split_payments_enabled: false
        output_dir = tmpdir / "amazon" / "transaction_matches"
        output_files = list(output_dir.glob("*_amazon_matching_results.json"))
        assert len(output_files) > 0
        ...
        assert data["metadata"]["split_payments_enabled"] is False
