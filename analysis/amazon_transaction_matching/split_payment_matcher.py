...
class SplitPaymentMatcher:
    ...
    def clear_order_matches(self, order_id: str):
        """
        Clear all matches for a specific order (useful for reprocessing).
        
        Args:
            order_id: Order ID to clear matches for
        """
        if order_id in self.matched_items:
            del self.matched_items[order_id]
        
        # Remove related transaction matches
        tx_to_remove = []
        for tx_id, match_info in self.transaction_matches.items():
            if match_info.get('order_id') == order_id:
                tx_to_remove.append(tx_id)
        
        for tx_id in tx_to_remove:
            del self.transaction_matches[tx_id]
        
        self.save_cache()
    
    def get_order_match_status(self, order_id: str, order_data: Dict) -> Dict:
        """
        Get the matching status for an order.
        ...
        Returns:
            Status dictionary with matched/unmatched items and amounts
        """
        matched_indices = self.matched_items.get(order_id, set())
        ...
        for i, item in enumerate(order_data.get('items', [])):
            if i in matched_indices:
                matched_amount += item.get('amount', 0)
            else:
                unmatched_amount += item.get('amount', 0)
        
        return {
            'order_id': order_id,
            'total_items': total_items,
            'matched_items': len(matched_indices),
            'unmatched_items': total_items - len(matched_indices),
            'matched_amount': matched_amount,
            'unmatched_amount': unmatched_amount,
            'is_fully_matched': len(matched_indices) == total_items,
